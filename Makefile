EXECUTABLES = chntpw-gui chntpw-gui-reset chntpw-gui-test
APPLICATIONS = chntpw-gui.desktop
POLICIES = org.chntpw.gui.policy
...
install: build
	...
	for MO_FILE in $(MO_FILES); do \
		...
		echo "Copying mo file $$MO_FILE to $(DESTDIR)/usr/share/locale/$$LOCALE/LC_MESSAGES/chntpw-gui.mo"; \
		install -Dm644 "$$MO_FILE" "$(DESTDIR)/usr/share/locale/$$LOCALE/LC_MESSAGES/chntpw-gui.mo"; \
	done
